...
#ifdef PTF_WINDOWS
        _PADDING = m_ColumnsContent_max_len[i];
#elif PTF_LINUX||PTF_UNIX||PTF_MAC
...
#ifdef PTF_WINDOWS
            int s1 = (PADDING_LEFT_RIGHT / 2 + m_ColumnsContent_max_len[i]) / 2 + (StringCapacity(str)) / 2;
			int s2 = m_ColumnsContent_max_len[i] - s1;
#elif PTF_LINUX||PTF_UNIX||PTF_MAC
